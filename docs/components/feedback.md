...
All six primitives are wired into `generateCode` with both HTML and JSX output. The dispatcher treats `toast`, `skeleton`, `spinner`, `kbd`, `progress`, and `meter` as first-class discriminants — they appear in `FAMILY_EMITTERS` and `SupportedType` (the 40-discriminant frozen table).
