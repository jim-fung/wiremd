...
- JSX rules (native elements only, no imports): `className` for classes; string attributes as double-quoted JSON string literals; boolean attributes emitted bare when true and omitted when false/absent; self-closing tags for void elements; children emitted recursively; JSX text escapes `& < > { }`.
